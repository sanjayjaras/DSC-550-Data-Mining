...
def calFuction(Xn, Ck, list_mu, list_cov):
    ...
    for i in range(num):
        tmp = 0
        for j in range(k):
            # print calculate_cluster_probability(Xn[i,:], list_mu[j], list_cov[j])
...
def update(X, c_k, list_mu, list_cov):
    ...
    for k_iter in range(k):
        ...
        for n_iter in range(num):
            tmp_var += r_zn_k[n_iter, k_iter] * (
                    (X[n_iter] - list_mu_new[k_iter]).transpose() * (X[n_iter] - list_mu_new[k_iter]))
...
with open('train.txt') as fin:
    ...
    # print calculate_cluster_probability(x1, mu1, var1)
    # print mu1.shape

    # print ck
    Ck_1, list_mu_1, list_cov_1 = init_para(4, 2, mu1, var1)
    print
    Ck_1, list_mu_1, list_cov_1
